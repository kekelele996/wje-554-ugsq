...
- 派单门禁：订单只能派给服务类目匹配且状态在线的技师，时段占用按服务时长计算，未取消、未评价订单之间的技师时段不得重叠；管理员改派失败（目标不合规、时段冲突或保存异常）时原技师与订单状态保持不动，成功后实时通知客户、原技师和新技师；派单面板展示每位候选技师的冲突对象与原因。
- 实时通知：后端 WebSocket Gateway 按接收人定向推送订单状态变更、新派单和技师出发通知。
