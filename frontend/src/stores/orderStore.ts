...
interface OrderState {
  ...
  loadAssignableWorkers: (id: string) => Promise<void>;
  updateStatus: (id: string, status: OrderStatus, workerId?: string) => Promise<void>;
  assign: (id: string, workerId: string) => Promise<void>;
...
export const useOrderStore = create<OrderState>((set, get) => ({
  ...
  loadAssignableWorkers: async (id) => set({ assignableWorkers: await orderApi.assignableWorkers(id) }),
  ...
  assign: async (id, workerId) => {
    const updated = await orderApi.assign(id, workerId);
    set({
      current: updated,
      orders: get().orders.map((order) => (order.id === id ? updated : order))
    });
